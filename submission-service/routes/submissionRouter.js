...
router.post("/", async (req, res) => {
  ...
  await Target.findById(req.body.targetId)
    .exec()
    .then(async (target) => {
      if (!target)
        throw {
          message: `Target with id:${req.body.targetId} could not be found.`,
          code: 404,
        };
    ...
    .then((savedSubmision) => {
      publisher.publishDirectExchange(
        { submission: savedSubmision },
        "direct_submission",
        "create"
      );
      res.status(201);
...
router.delete("/targets/:targetId", async (req, res) => {
  const submissionIds = (
    await Submission.find({ targetId: req.params.targetId })
  ).map((target) => target._id);
  if (submissionIds.length == 0) {
    res
      .status(200)
      .send("No submissions found for target: " + req.params.targetId);
    return res;
  }
  await Submission.deleteMany({ targetId: req.params.targetId })
    .exec()
    .then((deleteResult) => {
      if (deleteResult.deletedCount == 0) {
        throw new Error(
          "No submissions deleted for target: " + req.params.targetId
        );
      }
      ...
      res
        .status(200)
        .send(
          "All target submissions deleted for target id: " + req.params.targetId
        );
    })
    .catch((err) => {
      console.error(err);
...
router.delete("/user/:userId", async (req, res) => {
  const submissionIds = (
    await Submission.find({ createdByUser: req.params.userId })
  ).map((submission) => submission._id);
  if (submissionIds.length == 0) {
    res
      .status(200)
      .send("No submissions found for user id: " + req.params.userId);
    return res;
  }
  await Submission.deleteMany({ createdByUser: req.params.userId })
    .exec()
    .then((deleteResult) => {
      if (deleteResult.deletedCount == 0) {
        throw new Error(
          "No submissions deleted for user: " + req.params.userId
        );
      }
      for (const submissionId of submissionIds) {
        publisher.publishDirectExchange(
      ...
      res
        .status(200)
        .send("All submissions deleted for user: " + req.params.userId);
    })
    .catch((err) => {
      console.error(err);
      res.status(500).send("Removing submissions failed.");
