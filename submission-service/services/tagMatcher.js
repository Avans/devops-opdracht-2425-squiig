...
async function compareImage(target, submissionImage) {
    try {
        if (target.imageData === submissionImage) {
            throw {message:`Target image cannot be exactly the same as entry image.`, code: 400};
        }
        ...
        try {
            ...
            let score = compareTags(firstResponse.data.result.tags, secondResponse.data.result.tags);
            return score;
        } catch (error) {
...
function compareTags(targetTags, submissionTags) {
    ...
    for (const targetTag of targetTags) {
        const matchingSubmissionTag = submissionTags.find((tag) => tag.tag.en === targetTag.tag.en);
        if (matchingSubmissionTag) {
            matchingConfidence += matchingSubmissionTag.confidence;
        }
    }
    const totalConfidenceAllTags = targetTags.reduce((accumulator, tag) => accumulator + tag.confidence, 0);
    const percentage = Math.round((100 * matchingConfidence) / totalConfidenceAllTags);
